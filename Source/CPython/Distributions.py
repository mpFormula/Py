...
class Dist(object):
 ...
 #    Function FdisOwen(ByVal m As Long, ByVal n As Double, ByVal X As Double) As Double
    ...
    def cdisx_Canal(self, LeftTail, RightTail, n):
        ...
        stdev = math.sqrt(math.fabs(648 * m + 72 * m2 - 37 * m3)) / 108
        g = math.sqrt(0.5 * m3) / 162
        z = z - g + (z * g) * (z - (2 * z * z - 5) * g)
        L = 6 * (z * stdev + mean)
        h = self.cbrt(2 * (L + math.sqrt(13 + L * (L - 5))) - 5)
        U = 0.5 + 0.5 * h - 1.5 / h
        U = U * U * U
        result = n * U * U
        return result
    ...
    def demoCdisx(self):
        ...
        print("LambertL: ", r1, ", ", math.fabs(r1 - R0) / R0)
        r2 = self.cdisx_Canal(LeftTail, RightTail, m)
        print("Canal   : ", r2, ", ", math.fabs(r2 - R0) / R0)
        print("")
